...
# Define a Prefect task to download the US traffic accidents dataset from Kaggle and read it into a Pandas DataFrame
...
def write_gcs(parquet_path: Path) -> None:
    ...
    gcp_cloud_storage_bucket_block = GcsBucket.load("us_accidents_data_lake_github-archive-de")
...
def write_bq(path: Path) -> None:
    ...
    gcp_credentials_block = GcpCredentials.load("gcs-credential-dataengineering")

    df_final.to_gbq(
        destination_table="github-archive-de.us_traffic_accidents_data",
